...
namespace Game
{
    public class ActionStack : MonoBehaviour
    {
        ...
        public T GetAction<T>() where T : class, IAction
        {
            foreach (IAction action in m_actionStack)
            {
                if (action is T result)
                {
                    return result;
                }
            }
        ...
        {
            return action != null && m_actionStack.Contains(action);
        }
